...
export class OnTooltipComponent implements OnInit {
  ...
  removeSomeTagsHTMLToDescription() {
    let tag = "";
    this.description = this.description.replace(/[<]/g, "&lt;").replace(/[>]/g, "&gt;");
    for (let types = 0; types < 2; types++) {
      ...
      for (let count = 0; words && count < words.length; count++) {
        ...
        if (this.description.indexOf(words[count]) !== -1) {
          this.description = this.description.replace(words[count], tag + words[count] + tag.replace("<", "</"));
        }
      }
    }
  }
